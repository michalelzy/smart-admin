...
//    private String contactPhone;
    ...
    @Length(max = 200, message = "站点名称最多200字符")
    private String stationName;

    @Schema(description = "设备名称")
    @NotBlank(message = "设备名称不能为空")
    @Length(max = 200, message = "设备名称最多200字符")
    private String deviceName;

    @Schema(description = "设备序列号")
    @NotBlank(message = "设备序列号不能为空")
    @Length(max = 200, message = "设备序列号最多200字符")
    private String serialNumber;

    @Schema(description = "版本号")
    @NotBlank(message = "版本号不能为空")
    @Length(max = 200, message = "版本号最多200字符")
    private String versionNumber;

    @Schema(description = "设备型号")
    @NotBlank(message = "设备型号不能为空")
    @Length(max = 200, message = "设备型号最多200字符")
    private String deviceModel;

    @Schema(description = "dtu序列号")
    @NotBlank(message = "dtu序列号不能为空")
    @Length(max = 200, message = "设dtu序列号最多200字符")
    private String dtuNumber;

    @SchemaEnum(desc = "类型", value = DeviceTypeEnum.class)
    @CheckEnum(message = "类型不正确", value = DeviceTypeEnum.class)
