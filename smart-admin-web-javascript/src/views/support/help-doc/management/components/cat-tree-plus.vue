...
<template>
  <a-card class="tree-container" size="small">
    ...
    <a-tree v-if="!_.isEmpty(helpDocCatalogTreeData)" v-model:selectedKeys="selectedKeys"
      ...
      :selectable="!props.checkable" :defaultExpandAll="false" @select="treeSelectChange">
      //TODO:
...
// ----------------------- 以下是暴露的方法内容 ----------------------------
// ----------------------- selectedKeys 中被传入了该节点的 help_doc_catalog_id，并给到父组件，父组件再传递给详情组件
// 去使用（也就是给到 help-doc-list.vue 去使用这个 help_doc_catalog_id 并加载他对应的内容） -----
defineExpose({
  queryHelpDocCatalogTree,
