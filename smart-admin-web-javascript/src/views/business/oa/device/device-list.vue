...
-->
<template>
  <!-- 根据当前用户的权限（如是否拥有 oa:device:query 权限），决定所在元素（这里是 <a-form>）是否显示、禁用或隐藏。 -->
  <a-form class="smart-query-form" v-privilege="'oa:device:query'">
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        ...
        <a-button @click="exportExcel()" v-privilege="'oa:device:exportExcel'" type="primary">
          <template #icon>
            <FileExcelOutlined />
          </template>
          导出数据（带水印）
    ...
    <a-table :scroll="{ x: 'max-content' }" size="small" :dataSource="tableData" :columns="columns" rowKey="deviceId"
      ...
      <template #bodyCell="{ column, record, text }">
        ...
        <template v-if="column.dataIndex === 'enterpriseName'">
          <a-button type="link" @click="detail(record.enterpriseId)" :disabled="!$privilege('oa:device:detail')">
