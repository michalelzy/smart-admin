    ...
    >
      <template #bodyCell="{ column, record, text }">
        ...
        <template v-if="column.dataIndex === 'type'">
          ...
          <a-tag :color="$smartEnumPlugin.getColorByValue('DEVICE_TYPE_ENUM', text)" :key="text" style="display: block; text-align: center;">
            {{ $smartEnumPlugin.getDescByValue('DEVICE_TYPE_ENUM', text) }}
          </a-tag>
        </template>
        <template v-if="column.dataIndex === 'status'">
          <a-tag :color="$smartEnumPlugin.getColorByValue('STATUS_ENUM', text)" :key="text" style="display: block; text-align: center;">
            {{ $smartEnumPlugin.getDescByValue('STATUS_ENUM', text) }}
          </a-tag>
        </template>
        <template v-if="column.dataIndex === 'dtuStatus'">
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            <a-button @click="update(record.deviceId)" size="small" v-privilege="'oa:device:update'" type="link">编辑</a-button>
            <a-button @click="confirmDelete(record.deviceId)" size="small" danger v-privilege="'oa:device:delete'" type="link">删除</a-button>
          </div>
        </template>
      </template>
    </a-table>
