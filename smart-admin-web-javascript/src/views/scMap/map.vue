...
/**
 * shallowRef 是 Vue 3 提供的响应式 API 之一，用于创建一个 “浅层响应式” 的引用对象。
浅层响应式：只对引用本身的变化（即 .value 的赋值）做出响应，不会递归监听引用对象内部属性的变化。
对比 ref：ref 是 “深层响应式”，会递归监听对象内部所有属性的变化；而 shallowRef 性能更好，适合存储大型对象或 DOM 元素（无需监听内部变化）。
 */
